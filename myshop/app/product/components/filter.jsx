...
export default function Filter({filters, filter}) {
  ...
  return <div className="p-10 flex flex-col gap-5 max-md:p-5">
    ...
    <div className="p-5 gap-10 flex md:items-center max-md:flex-col max-md:gap-5">
      <div className="text-grey-1">Filter products:</div>
      {
        Object.keys(filter).map((e, i) => {
          return <div key={i} onClick={() => setFilterBy(e == filterBy ? '' : e)} className={` select-none cursor-pointer flex gap-2 ${filterBy === e ? ' text-red-1' : ' hover:text-grey-1'}`}>
    ...
    <div className=" border-b-2 border-white-3">
      {
        filterBy && <div className=" p-5 bg-white-2 flex gap-5 flex-wrap">
          {
            filter[filterBy].map((e, i) => {
              const isCheck = searchParams.has(filterBy, e)
              return <div onClick={() => handleFilter(filterBy, e)} key={i} className=" select-none cursor-pointer flex gap-2 items-center hover:text-grey-1">
    ...
    <div className=" flex gap-5 flex-wrap items-center">
      {
        filters.map((e, i) => {
          return <div className=" py-2 px-5 border-2 border-white-3 rounded-3xl text-xs flex gap-2 items-center" key={i}>
            {e[1]}
