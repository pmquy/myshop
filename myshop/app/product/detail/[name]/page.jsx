...
function Designer({ name }) {
  ...
  const query = useQuery({
    ...
    queryFn: () => API.DesignerApi.findOne({ name: name })
  })
  ...
  return <div className="flex flex-col p-10 gap-5 bg-black-1 text-white-1 items-center">
    ...
    <div className="text-2xl font-semibold">{designer.name}</div>
    <div className=" flex gap-2">
      {designer.images.map((e, i) => <img className={`h-60 ${i == 0 ? '' : 'max-xl:hidden'}`} key={e} src={e} />)}
    </div>
    <div className="text-center">{designer.description}</div>
    <Link href={'/product/designer/' + designer.name}><div className=" border-y-2 border-white-3 md:w-max px-10 py-2 btn hover:bg-white-1 hover:text-black font-semibold transition-colors">LEARN MORE</div></Link>
  </div>
}
...
function Introduction({ product }) {
  return <div className=" p-10 flex items-center max-lg:flex-col max-lg:gap-10">
    ...
    <div className=" basis-1/2 flex flex-col gap-5 max-lg:items-center">
      ...
      <div className=" text-2xl text-grey-1">{product.designer}</div>
...
export default function Page() {
  ...
  const query = useQuery({
    ...
    queryFn: () => API.ProductApi.findOne({ name: name })
  })
  ...
  return <div className=" flex flex-col gap-10">
    ...
    <div id="designer"><Designer name={product.designer} /></div>
  </div>
}
